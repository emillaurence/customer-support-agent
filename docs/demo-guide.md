...
Other seeded customers (`ada@example.com`, `sofia@example.com`) exist to exercise edge cases in tests — an expired return window, an in-transit order, multiple active orders requiring clarification — but aren't needed for the three scenarios below.

## Scenario 1: Order discovery + clarification
...
Verify as Bruce (`bruce@example.com`), then ask about order `ORD-1003`.
...
Verify as Kenji (`kenji@example.com`) and ask about returning items on both of his orders.
